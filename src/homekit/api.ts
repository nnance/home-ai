...
export declare interface HomebridgeAPI {
  ...
  on(
    event: InternalAPIEvent.REGISTER_ACCESSORY,
    listener: (
      accessoryName: AccessoryName,
  ...
  on(
    event: InternalAPIEvent.REGISTER_PLATFORM,
    listener: (
      platformName: PlatformName,
      platformConstructor: PlatformPluginConstructor,
      pluginIdentifier?: PluginIdentifier
    ) => void
  ): this;

  on(
    event: InternalAPIEvent.PUBLISH_EXTERNAL_ACCESSORIES,
    listener: (accessories: PlatformAccessory[]) => void
  ): this;
  on(
    event: InternalAPIEvent.REGISTER_PLATFORM_ACCESSORIES,
    listener: (accessories: PlatformAccessory[]) => void
  ): this;
  on(
    event: InternalAPIEvent.UPDATE_PLATFORM_ACCESSORIES,
    listener: (accessories: PlatformAccessory[]) => void
  ): this;
  on(
    event: InternalAPIEvent.UNREGISTER_PLATFORM_ACCESSORIES,
    listener: (accessories: PlatformAccessory[]) => void
  ): this;
...
export class HomebridgeAPI extends EventEmitter implements API {
  ...
  public static isDynamicPlatformPlugin(
    platformPlugin: PlatformPlugin
  ): platformPlugin is DynamicPlatformPlugin {
    return "configureAccessory" in platformPlugin;
  }

  public static isStaticPlatformPlugin(
    platformPlugin: PlatformPlugin
  ): platformPlugin is StaticPlatformPlugin {
    return "accessories" in platformPlugin;
  }

  public static isQualifiedPluginIdentifier(identifier: string): boolean {
    return HomebridgeAPI.PLUGIN_IDENTIFIER_PATTERN.test(identifier);
  }
  ...
  registerAccessory(
    pluginIdentifier: PluginIdentifier | AccessoryName,
    accessoryName: AccessoryName | AccessoryPluginConstructor,
    constructor?: AccessoryPluginConstructor
  ): void {
  ...
  registerPlatform(
    pluginIdentifier: PluginIdentifier | PlatformName,
    platformName: PlatformName | PlatformPluginConstructor,
    constructor?: PlatformPluginConstructor
  ): void {
  ...
  publishExternalAccessories(
    ...
    if (!HomebridgeAPI.isQualifiedPluginIdentifier(pluginIdentifier)) {
      log.info(
        `One of your plugins incorrectly registered an external accessory using the platform name (${pluginIdentifier}) and not the plugin identifier. Please report this to the developer!`
      );
    }
    ...
    accessories.forEach((accessory) => {
      ...
      if (!(accessory instanceof PlatformAccessory)) {
        throw new Error(
          `${pluginIdentifier} attempt to register an accessory that isn't PlatformAccessory!`
        );
      }
  ...
  registerPlatformAccessories(
    ...
    accessories.forEach((accessory) => {
      ...
      if (!(accessory instanceof PlatformAccessory)) {
        throw new Error(
          `${pluginIdentifier} - ${platformName} attempt to register an accessory that isn't PlatformAccessory!`
        );
      }
